define([
    ...
    MpLinkHandler, ErrorHandler, ConfigBuilder, ConfigDialogue, dragAndDropCapability, Enum) {
    ...
    p.getChildrenData = function () {
        ...
        for (var i = 0; i < storedItems.childrenIdList.length; i += 1) {
            items.push({
                type: brease.callWidget(storedItems.childrenIdList[i], 'getShortColumnType'),
                width: brease.callWidget(storedItems.childrenIdList[i], 'getWidth')
            });
    ...
    */
    ...
    return dragAndDropCapability.decorate(MeasurementSystemDependency.decorate(CultureDependency.decorate(WidgetClass, true), true), false);
});
